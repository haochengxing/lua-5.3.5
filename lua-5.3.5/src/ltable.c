...
/* 为了减少空表的维护成本,Lua在这里做了一点优化。宏定义了一个不可改写的空哈希表:dummynode */
...
static void setnodevector (lua_State *L, Table *t, unsigned int size) {
  if (size == 0) {  /* no elements to hash part? */
    t->node = cast(Node *, dummynode);  /* use common 'dummynode' 让空表被初始化时,node域指向这个dummynode节点。它虽然是一个全局变量, 但因为对其访问是只读的,所以不会引起线程安全问题  */
    t->lsizenode = 0;
...
TValue *luaH_newkey(lua_State *L, Table *t, const TValue *key) {
	...
	mp = mainposition(t, key); //拿到key 可以存放的的node lua的哈希表以闭散列方式实现。每个可能的键值,在哈希表中都有一个主要位置,称作mainposition
	/* 如果存在 */
	if (!ttisnil(gval(mp)) || isdummy(t)) {  /* main position is taken? */
		Node *othern;
		Node *f = getfreepos(t);  /* 扩容 get a free place */
		if (f == NULL) {  /* cannot find a free place? */
			rehash(L, t, key);  /*  扩容 grow table   在getfreepos递减到哈希空间头时,做一次rehash操作 */
		...
		if (othern != mp) {  /* is colliding node out of its main position?    此处需要解决hash冲突*/
		  /* yes; move colliding node into free position */
			while (othern + gnext(othern) != mp)  /* find previous  若两者位置冲突,则利用Node结构中的next域, 以一个单向链表的形式把它们链起来 */
				othern += gnext(othern);
	...
	setnodekey(L, &mp->i_key, key); //拷贝到node上  创建一个新键时,检查mainposition,若无人使用,则可以直接设置为这个新键。
...
*  当查询键为整数键且在数组范围内时，在数组部分查询；否则，根据键的哈希值去哈希表中查询。
*  拥有相同哈希值的冲突键值对，在哈希表中由Node的next域单向链起来，所以遍历这个链表就可以了。
*/
